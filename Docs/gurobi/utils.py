...
def print_h_i_file(x_ijk,h_i,l_i, y_ij, psi_jk, delta_k, vl_k, customers, lockers, locker_attrib, vehicles, qty_vehicles,instance,m,nodes, time):

    file = None

    if qty_vehicles == 1:
        file = open(f"output/{instance['inst_name'].split('.')[0]}.md",'w')
        file.writelines(["$h_i$ - Cliente $i$ foi visitado em casa\n",
                        "$l_i$ - Cliente $i$ foi visitado no locker\n",
                        "$y_{ij}$ - Cliente $i$ foi atendido no locker $j$\n",
                        "$\ell_{ij} $ - Cliente $i$ esta atribuido ao locker $j$\n",
                        "$\psi_{jk} $ - Quantidade de pacotes deixados no locker $j$ pelo veiculo $k$\n",
                        "$\delta_{k} $ - Momento de saida do veiculo $k$ do deposito\n",
                        "$\mathcal{v}_{k} $ - Momento em que o veiculo $k$ visitou o ultimo cliente\n"])
    else:
        file = open(f"output/{instance['inst_name'].split('.')[0]}.md",'a')
    
    file.writelines(f"- **qty_v: {qty_vehicles}, cost: {'Infeasible' if m.status == gp.GRB.INFEASIBLE else m.ObjVal}, time: {round(time,2)}s**\n")
    

    if m.status != gp.GRB.INFEASIBLE:
    
        file.writelines("**Route:**\n")    
        for vehicle in vehicles:
            file.writelines(f"  - ${vehicle}$: ")
            for node_i in nodes:
                for node_j in nodes:
                    if round(x_ijk[node_i,node_j,vehicle].X) == 1:
                        file.writelines(f"(${node_i},{node_j}$) ")
            file.writelines("\n")
            
        
        table = f"\n|var|{'|'.join(customer for customer in customers)}|\n |---|---{'---'.join('|' for customer in customers)}\n"
        line_h_i = "|$h_i$"
        line_l_i = "|$l_i$"
        line_y_ij = "|$y_{ij}$"
        line_l_ij = r"|$\ell_{ij}$"
        ...
        file.write(f"{table}{line_h_i}{line_l_i}{line_y_ij}{line_l_ij}\n")

        
        
        
        table = "|$\psi_{jk}$|"+f"{'|'.join(locker for locker in lockers)}|\n |---|---{'---'.join('|' for locker in lockers)}\n"
        lines_psi_jk = ""
        ...
        for k in vehicles:
            ...
            lines_psi_jk+="\n"
        file.write(f"{table}{lines_psi_jk}\n")
        ...
        line_vl = ""
        for v in vehicles:
            table_delta += "| $\delta_{"+v+"}$ "
            table_vl += "| $\mathcal{v}_{"+v+"}$ "
            line_delta+= f"| {delta_k[v].X}"
            line_vl+= f"| {vl_k[v].X}"
        table_delta+=f"\n |--{'----'.join('|' for v in vehicles)}\n"
        table_vl+=f"\n |--{'----'.join('|' for v in vehicles)}\n"
        ...
        file.write(f"{table_delta}{line_delta}\n")
        
        
        
        file.write(f"{table_vl}{line_vl}\n")
        file.close()
