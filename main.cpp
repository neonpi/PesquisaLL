...
int main()
{
    ...
    for(Instance* instance: instances) {
        if (instance->inst_name != "R103_co_100.txt") {
            //if (instance->inst_name == "C103_co_25.txt") {
            ...
            for(int i=0;i<config->runs;i++) {
                ...
                Utils::print_route_file(search,i==0);
                search->print_is_viable();
                delete search;
            }

            stats->finish_stats();
            cout<<"AVG_COST: "<<stats->avg_cost<<" - AVG_TIME: "<<stats->avg_time<<" - BEST_COST: "<<stats->best_cost<<" - BEST_TIME: "<<stats->avg_time<<endl;
